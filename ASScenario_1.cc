...
int main (int argc, char *argv[]) {
 ...
 // CONFIG PARAMS
  ...
  uint16_t NenbNode = 1;    // NUMBER OF ENBS (MIN = PLMNS)--> MIN=PLMNS????
...
// FIXED PARAM
  ...
  /*===-------DOUBT-----=====
  epo = NenbNode/plmns
  epo can be zero only if NenbNode=0, 
  epo cannot be less than zero, since both NenbNode and plmns should be positive whole numbers
  This case should be dealt with during the error correction phase
  */

  
  
  /*===========SOME REDUCTION OF VARIABLE SPACE===============
  
  if epo>0 => NenbConnections = NenbConnections * NenbNode
  Now, cpe = NenbConnections/NenbNode = original(NenbConnections)
       cpo = NenbConnections/plms = original(NenbConnections) * NenbNodes/plmns
...
if (remotehosts > NueNode){
 ...
 if (cpe > plmns){
  ...
  //**************NODE CONTAINERS INITIALIZED HERE**************
 ...
 for ( uint16_t u = 0; u < NueNode; u++) {
   ...
   if ( epo > 0 ){
    // EPO > 0
     ...
     int ceb = u % iterator;
     int connused = ceb / cpe;
     int internal = ceb % cpe;
     int enb = u / upe;        
