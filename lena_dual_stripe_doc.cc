...
/*---------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------*/
//This function checks if the two boxes provided have overlapping boundaries or not.
//Returns a boolean value from the above check
...
----------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------*/
//A class of femtocell block allocators
...
public:
  ...
  void Create (uint32_t n);                    //for when more than 1 femtocells are needed to be created
  void Create ();
...
//Defining methods of Class FemtocellBlockAllocator outside the class definition
//-------------------------------------------------------------------------------
//-----------------START OF METHODS OF FEMTOCELLBLOCKALLOCATOR--------------------
//--------------------------------------------------------------------------------

//Constructor for the class objects

//C++ NOTE-An initialization list is necessary if one of the fields is a reference because a reference can never be null, not even in the brief time between object construction and the body of the constructor. The following raises error C2758

//Initialization List is used to initialize objects and other simple data members
//Pointer members are initialized through the constructor
...
//Member Function to Create multiple femtocells
void 
...
//Member function to create a single femtocell
void
FemtocellBlockAllocator::Create ()
{
  ...
  //creating boxes to be allocated to different femtocells while boxes overlap
  
  //creating the buildings-map for allocating femtocells to each building or area
  ...
  gridBuildingAllocator->Create (2);//creates buildings acc. to the specifications given above
}
...
//-----------------------------------------------------------------------------------------------------
//------------------------END OF METHODS OF FEMTOCELLBLOCKALLOCATOR------------------------------------
//-----------------------------------------------------------------------------------------------------


//Probably, saving a log file for buildings
void 
...
//Probably saving a log file for UEs
void 
...
//Probably saving a log file for En
...
// Setting up LteHelper for the scenario
...
// Setting up EPC
/* The EPC contains entities like SGW and MSGN and PDGN
   The EPC is responsible for connecting calls to other networks
*/
...
// Macro eNode-Bs have been setup in the above lines, with different parameters

// COnfigure EPC to enable handover over X2
  if (epc)
    {
  ...
  // HomeEnbs are configured with their own parameter set
  ...
  //Setting the mobility model for the macro UEs for both moving and stationary versions
