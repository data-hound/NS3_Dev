...
int main (int argc, char *argv[]) {
 ...
 // CONFIG PARAMS
  ...
  uint16_t NHenbNode = 1; // NUMBER OF HENBS
...
if (remotehosts > NueNode){
 ...
 for ( uint16_t u = 0; u < NueNode;) {
  ...
  tempu0++; //after u is incremented, the incremented value is not stored in tempu0
       ...
       //int Henb_idx_overhead;
        ...
        for(uint16_t p = 0 ; p < plmns ; ){
          for(uint16_t c = 0 ; c < cpo ; c++){
           ...
           epcHelper->AssignEnbToOperator(epcipaddress[conn], HenbNode, HenbLteDevs.Get(Henb), enbInf[Henb].lteSocket, enbLteDevs.Get(Henb)->GetObject<ASLteEnbNetDevice> ()->GetCellId (), enbeth[Henb], enbMacAddress[Henb], enbInf[Henb].enbApp, (p+c) % plmns);
           //Error in line 550 for HeNBs
           //assert failed. cond="(m_network & ~m_mask) == 0
           //msg="Ipv4AddressHelper::SetBase(): Inconsistent network and mask", file=../src/internet/helper/ipv4-address-helper.cc, line=75
           //in line 334 of ASP2PEpcHelper Ipv4AddressHelper::SetBase() is used
           //The ip address of epc that is passed, is masked with 255.255.0.0 by default. This masking is causing problems.
           conn++;
          }
          p++;
        }
